        ...
        const fetchData = async () => {
            try {
                ...
                transformedMembers.sort((a, b) => {
                    ...
                    const deptNameA = a.departmentName || "";
                    const deptNameB = b.departmentName || "";
                    if (deptNameA !== deptNameB) return deptNameA.localeCompare(deptNameB);

                    const memberOrderA = a.display_order || 0;
                    const memberOrderB = b.display_order || 0;
    ...
    return (
        <div className="neural-teams-page">
            ...
            <div className="container-neural">
                ...
                {/* Grouped Team Members Section */}
                {Object.entries(
                    teamMembers.reduce((acc, member) => {
                        const key = member.teamTitle || "Other";
                        if (!acc[key]) acc[key] = [];
                        acc[key].push(member);
                        return acc;
                    }, {})
                ).map(([teamTitle, members]) => (
                    <React.Fragment key={teamTitle}>
                        <div className="section-label">CLUSTER_NODES // {teamTitle.toUpperCase()}</div>
